...
class InforGo(object):
    ...
    def __init__(self, player_len=1, pattern_len=6, n_hidden_layer=1, n_node_hidden=[32], activation_fn='tanh', learning_rate=0.001, directory='../../Data/default/', alpha=0.1, gamma=0.99, lamda=0.85, search_depth=3, c=1, n_playout=10000, playout_depth=10, play_first=True, tree_type='minimax', eps=0.1):
        ...
        self._tree = minimax(search_depth, self.nn, eps) if tree_type == 'minimax' else mcts(lamda, c, n_playout, self.nn, playout_depth, player)
    ...
    def refresh(self):
        self._tree = None
        player = 1 if self._play_first else -1
        self._tree = minimax(self._search_depth, self.nn, self._eps) if self._tree_type == 'minimax' \
                                                                  else mcts(self._lamda, self._c, self._n_playout, self.nn, self._playout_depth, player)
    def step(self, act):
        self._tree.step(act)
